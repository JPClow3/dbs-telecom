...
export const ReferralModal: React.FC<ReferralModalProps> = ({
  ...
  const loadSummary = async () => {
    ...
    try {
      ...
      onShowToast('Erro ao carregar indicações: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCopyLink = () => {
    ...
    onShowToast('Link de indicação copiado!');
  };
  ...
  const handleAddFriend = async () => {
    ...
    if (!friendName.trim() || !friendPhone.trim()) {
      onShowToast('Preencha o nome e WhatsApp do amigo.');
      return;
    }
          ...
          {loading ? (
            ...
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
              ...
              <View style={[styles.shareCard, { backgroundColor: colors.background, borderColor: colors.borderLight }]}>
                ...
                <View style={styles.shareButtonsRow}>
                  <TouchableOpacity
                    ...
                    accessibilityLabel={isDemo ? 'Copiar (demo) — indisponível' : 'Copiar link'}
                  >
                    <Copy size={16} color={isDemo ? colors.textMuted : colors.text} />
                    <Text style={[styles.copyBtnText, { color: isDemo ? colors.textMuted : colors.text }]}>{isDemo ? 'Copiar (demo)' : 'Copiar Link'}</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    disabled={isDemo}
                    accessibilityRole="button"
                    accessibilityLabel={isDemo ? 'Compartilhar (demo) — indisponível' : 'Compartilhar no WhatsApp'}
                  >
                    <Share2 size={16} color={isDemo ? colors.textMuted : '#FFFFFF'} />
                    <Text style={[styles.whatsappBtnText, isDemo && { color: colors.textMuted }]}>{isDemo ? 'Compartilhar (demo)' : 'Compartilhar no WhatsApp'}</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <TouchableOpacity
                ...
                disabled={isDemo}
                accessibilityRole="button"
                accessibilityLabel={isDemo ? 'Indicar amigo (demo) — indisponível' : showAddForm ? 'Fechar formulário' : 'Indicar amigo por nome ou WhatsApp'}
              >
