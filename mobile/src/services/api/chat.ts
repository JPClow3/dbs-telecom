...
import { getApiUrl, getAuthHeaders, isApiServiceError, responseError, unavailableError, unavailableMessage } from './transport';
...
export async function getInitialGreeting(clientId: string): Promise<ChatMessage> {
  ...
  try {
    const res = await fetch(`${getApiUrl()}/chat/greeting`, {
      method: 'POST',
...
export async function sendMessage(message: string, sessionId: string, clientId?: string): Promise<ChatMessage> {
  if (isDemoMode()) {
