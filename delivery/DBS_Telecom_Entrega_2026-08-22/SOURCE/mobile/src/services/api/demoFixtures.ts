...
// Fixtures dos adaptadores de demonstração (Wi-Fi/TR-069, telemetria óptica,
// notificações e indicações). Só são usados com o modo demo explicitamente
// habilitado; em ambiente real as chamadas seguem para a API autenticada.
export function buildDemoWifiSettings(clientId: string): WifiSettings {
  return {
    clientId,
    ssid2G: 'DBS-FIBRA-2G',
    ssid5G: 'DBS-FIBRA-5G',
    password: 'demo12345',
    guestSsid: 'DBS-VISITAS',
    guestPassword: 'visitas123',
    guestEnabled: true,
...
export function buildDemoOpticalDiagnostics(clientId: string): OpticalDiagnosticResult {
  return {
    clientId,
    rxPowerDbm: -19.4,
    txPowerDbm: 2.1,
    ...
    statusLabel: 'Sinal dentro da faixa ideal',
    description: 'Atenuação de -19.4 dBm está dentro da faixa ideal (-15 a -24 dBm) para FTTH residencial.',
    recommendation: 'Nenhuma ação necessária. Seu link óptico está saudável.',
    ticketCreated: false,
...
export const MOCK_NOTIFICATIONS: PushNotification[] = [
  {
    ...
    title: 'Fatura de setembro disponível',
    body: 'Sua fatura no valor de R$ 119,90 vence em 10/09. Pague via PIX em 1 clique.',
    actionType: 'COPY_PIX',
    actionPayload: 'DEMO-PIX-NÃO-PAGAR',
    read: false,
  ...
  },
  {
    ...
    clientId: 'demo-2270',
    type: 'TICKET_STATUS',
    title: 'Chamado DBS-781920 em andamento',
    body: 'Técnico a caminho para troca do roteador Wi-Fi 6.',
  ...
  },
  {
    ...
    body: 'Um amigo assinou com seu link. 50% de desconto na próxima fatura.',
    actionType: 'VIEW_REFERRALS',
    read: true,
    createdAt: new Date(Date.now() - 86400_000).toISOString(),
  },
];

export function buildDemoReferralSummary(clientId: string): ReferralSummary {
  return {
    clientId,
    ...
    activeDiscounts: 1,
    totalSaved: 119.9,
    totalSavedFormatado: 'R$ 119,90',
    friends: [
      {
        id: 'demo-friend-1',
        name: 'Ana Clara Souza',
        phone: '(49) 99999-0001',
        status: 'ACTIVE_DISCOUNT',
        statusLabel: 'Desconto Ativo',
        statusBadgeColor: '#16A34A',
        discountMonth: 'Setembro 2026',
      ...
      },
      {
        ...
        statusLabel: 'Aguardando Instalação',
