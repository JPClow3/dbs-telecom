...
export const QueueCard: React.FC<QueueCardProps> = ({
  ...
  // Mantém o callback mais recente acessível dentro da inscrição SSE sem
  // reinscrever a cada render (evita closure obsoleta).
  const advanceRef = useRef(onAdvanceQueue);
  useEffect(() => {
    advanceRef.current = onAdvanceQueue;
  }, [onAdvanceQueue]);
  ...
  const handleAdvance = async () => {
    ...
    try {
      ...
      if (res?.entry) {
        ...
        onShowToast?.('Não foi possível avançar a fila agora.', 'WARNING');
      }
    } catch (e) {
      console.warn('Erro ao avançar fila:', e);
      onShowToast?.('Falha ao avançar a fila. Tente novamente.', 'WARNING');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      onShowToast?.('Não foi possível sair da fila agora. Tente novamente.', 'WARNING');
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            ...
            <View style={styles.agentInfo}>
              <Text style={[styles.agentName, { color: colors.text }]}>
                {queue.assignedAgent?.name || 'Atendente DBS Telecom'}
              </Text>
              <Text style={[styles.agentRole, { color: colors.textSecondary }]}>
                {queue.assignedAgent?.role || 'Especialista em Atendimento'}
              </Text>
            </View>
          </View>
          <Text style={[styles.assignedMessage, { color: colors.textSecondary }]}>
            "Olá! Aqui é da equipe de atendimento da DBS Telecom. Como posso te auxiliar com sua solicitação hoje?"
          </Text>
        </View>
      )}
