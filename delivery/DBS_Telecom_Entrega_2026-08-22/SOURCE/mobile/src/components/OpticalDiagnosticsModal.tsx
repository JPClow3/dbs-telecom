...
export const OpticalDiagnosticsModal: React.FC<OpticalDiagnosticsModalProps> = ({
  ...
  const runDiagnosis = async (simulatedRx?: number) => {
    ...
    try {
      ...
      setData(null);
      setLoadError(true);
      onShowToast('Erro ao ler potência ótica: ' + e.message, 'WARNING');
    } finally {
      setLoading(false);
    }
  };
  ...
  const rxVal = data?.rxPowerDbm ?? -35;
        ...
        <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.header, { borderBottomColor: colors.borderLight }]}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel="Fechar telemetria de sinal"
            >
            ...
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
              ...
              {loadError && !isDemo && (
                <View style={[styles.demoNotice, { backgroundColor: colors.warningLight, borderColor: colors.warningBorder }]}>
                  <AlertTriangle size={16} color={colors.warningDark} />
                  <Text style={[styles.demoNoticeText, { color: colors.warningDark }]}>
                    Não foi possível obter a telemetria do servidor. As leituras abaixo não representam o seu sinal real.
                  </Text>
                  <TouchableOpacity
                    style={[styles.retryBtn, { borderColor: colors.warningDark }]}
                    onPress={() => runDiagnosis()}
                    accessibilityRole="button"
                    accessibilityLabel="Tentar medir o sinal óptico novamente"
                  >
              ...
              <View style={[styles.gaugeCard, { backgroundColor: colors.background, borderColor: colors.border }]}>
                ...
                <Text style={[styles.gaugeValue, { color: badge?.text }]}>
                  {data ? <>{data.rxPowerDbm} <Text style={styles.unitText}>dBm</Text></> : 'Indisponível'}
                </Text>
              ...
              {data?.ticketCreated && !isDemo && (
                <View style={[styles.ticketBanner, { backgroundColor: colors.dangerLight, borderColor: colors.dangerBorder }]}>
                  ...
                  <TouchableOpacity
                    ...
                    accessibilityRole="button"
                    accessibilityLabel="Acompanhar chamado preventivo no chat de suporte"
                  >
              ...
              {/* Testes de Simulação — exclusivos do ambiente de demonstração.
                  Em produção, valores fabricados nunca são enviados como
                  telemetria real. */}
              {isDemo && (
