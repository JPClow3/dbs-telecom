...
export const WifiManagerModal: React.FC<WifiManagerModalProps> = ({
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setSettings(null);
      setLoadError(true);
      onShowToast('Falha ao carregar Wi-Fi: ' + (e.message || 'Erro de conexão'), 'WARNING');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!settings) {
      onShowToast('Nenhuma configuração confirmada foi carregada. Tente sincronizar novamente antes de alterar o Wi-Fi.', 'WARNING');
      return;
    }
    if (!ssid2G.trim() || !ssid5G.trim()) {
      onShowToast('Informe o nome (SSID) das redes principal e turbo.', 'WARNING');
      return;
    }
    if (guestEnabled && !guestSsid.trim()) {
      onShowToast('Informe o nome da rede de visitas.', 'WARNING');
      return;
    }
    if (password.length < 8) {
      onShowToast('A senha principal deve ter no mínimo 8 caracteres.', 'WARNING');
      return;
    }
    if (guestEnabled && guestPassword.length < 8) {
      onShowToast('A senha da rede de visitas deve ter no mínimo 8 caracteres.', 'WARNING');
      return;
    }
  ...
  const handleCopyValue = async (text: string, label: string) => {
    ...
    if (result.copied) {
      ...
      onShowToast(`Não foi possível copiar ${label.toLowerCase()}.`, 'WARNING');
    }
  };

  const handleShareGuestOnWhatsApp = () => {
    if (isDemo) {
      onShowToast('Prévia local: compartilhamento de Wi-Fi está desativado.');
      return;
    }
    hapticFeedback.medium();
    const text = `Olá! Conecte-se ao Wi-Fi de Visitas da DBS Telecom:\n\n📶 Rede: ${guestSsid}\n🔑 Senha: ${guestPassword}`;
        ...
        <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.header, { borderBottomColor: colors.borderLight }]}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel="Fechar gerenciador Wi-Fi"
            >
            ...
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
              ...
              {loadError && !isDemo && (
                <View style={[styles.demoNotice, { backgroundColor: colors.warningLight, borderColor: colors.warningBorder }]}>
                  <AlertTriangle size={16} color={colors.warningDark} />
                  <Text style={[styles.demoNoticeText, { color: colors.warningDark }]}>
                    Não foi possível sincronizar com o roteador. Os campos abaixo estão vazios porque nenhuma configuração real foi carregada.
                  </Text>
                  <TouchableOpacity
                    style={[styles.retryBtn, { borderColor: colors.warningDark }]}
                    onPress={loadSettings}
                    accessibilityRole="button"
                    accessibilityLabel="Tentar sincronizar Wi-Fi novamente"
                  >
              ...
              {/* Status da Rede */}
              <View style={[styles.statusBanner, { backgroundColor: (isDemo || loadError) ? colors.warningLight : colors.primaryUltraLight, borderColor: (isDemo || loadError) ? colors.warningBorder : colors.primaryBorder }]}>
                <View style={styles.statusRow}>
                  <Radio size={16} color={(isDemo || loadError) ? colors.warningDark : colors.primary} />
                  <Text style={[styles.statusText, { color: (isDemo || loadError) ? colors.warningDark : colors.primary }]}>
                    {isDemo
                      ? `Prévia: status do roteador não confirmado • ${settings?.connectedDevices || 5} aparelhos ilustrativos`
                      : loadError || !settings
                        ? 'Status do roteador indisponível — nenhuma configuração foi confirmada'
              ...
              <View style={[styles.sectionBlock, { borderColor: colors.borderLight }]}>
                ...
                <View style={[styles.passwordWrapper, { backgroundColor: colors.background, borderColor: colors.border }]}>
                  ...
                  />
                  <TouchableOpacity
                    onPress={() => handleCopyValue(password, 'Senha do Wi-Fi')}
                    style={styles.eyeBtn}
                    accessibilityRole="button"
                    accessibilityLabel="Copiar senha do Wi-Fi principal"
                  >
                  ...
                  <TouchableOpacity
                    ...
                    style={styles.eyeBtn}
                    accessibilityRole="button"
                    accessibilityLabel={showPassword ? 'Ocultar senha do Wi-Fi principal' : 'Mostrar senha do Wi-Fi principal'}
                  >
              ...
              <View style={[styles.sectionBlock, { borderColor: colors.borderLight }]}>
                <View style={styles.sectionHeaderBetween}>
                  ...
                  <Switch
                    ...
                    thumbColor="#FFFFFF"
                    accessibilityLabel="Ativar ou desativar a rede de visitas"
                  />
                </View>
                ...
                {guestEnabled && (
                  <>
                    ...
                    <View style={[styles.passwordWrapper, { backgroundColor: colors.background, borderColor: colors.border }]}>
                      ...
                      />
                      <TouchableOpacity
                        onPress={() => handleCopyValue(guestPassword, 'Senha de Visitas')}
                        style={styles.eyeBtn}
                        accessibilityRole="button"
                        accessibilityLabel="Copiar senha da rede de visitas"
                      >
                        <Copy size={16} color={colors.textMuted} />
                      </TouchableOpacity>
                      <TouchableOpacity
                        onPress={() => setShowGuestPassword(!showGuestPassword)}
                        style={styles.eyeBtn}
                        accessibilityRole="button"
                        accessibilityLabel={showGuestPassword ? 'Ocultar senha da rede de visitas' : 'Mostrar senha da rede de visitas'}
                      >
              ...
              <View style={styles.actionsRow}>
                <TouchableOpacity
                  ...
                  disabled={rebooting || isDemo || loadError || !settings}
                  accessibilityRole="button"
                  accessibilityLabel={isDemo ? 'Reiniciar (demo) — indisponível' : (loadError || !settings) ? 'Reiniciar Wi-Fi — indisponível sem configuração confirmada' : 'Reiniciar Wi-Fi'}
                  activeOpacity={0.8}
                >
                  <RefreshCw size={16} color={(isDemo || loadError || !settings) ? colors.textMuted : colors.text} />
                  <Text style={[styles.rebootBtnText, { color: colors.text }]}>
                    {rebooting ? 'Reiniciando...' : isDemo ? 'Reiniciar (demo)' : loadError || !settings ? 'Reiniciar indisponível' : 'Reiniciar Wi-Fi'}
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  onPress={handleSave}
                  disabled={saving || isDemo || loadError || !settings}
                  accessibilityRole="button"
                  accessibilityLabel={isDemo ? 'Salvar (demo) — indisponível' : (loadError || !settings) ? 'Salvar Wi-Fi — indisponível sem configuração confirmada' : 'Salvar alterações'}
                  activeOpacity={0.85}
                >
                  <CheckCircle2 size={16} color={(isDemo || loadError || !settings) ? colors.textMuted : '#FFFFFF'} strokeWidth={2.5} />
                  <Text style={styles.saveBtnText}>
                    {saving ? 'Aplicando...' : isDemo ? 'Salvar (demo)' : loadError || !settings ? 'Salvar indisponível' : 'Salvar Alterações'}
                  </Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
              ...
              <View style={[styles.qrCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.qrCardHeader}>
                  ...
                  <TouchableOpacity
                    ...
                    accessibilityRole="button"
                    accessibilityLabel="Fechar QR Code de visitas"
                  >
                    <X size={20} color={colors.textMuted} />
                  </TouchableOpacity>
                </View>
                ...
                <View style={styles.qrActions}>
                  <TouchableOpacity
                    ...
                    onPress={() => handleCopyValue(guestPassword, 'Senha')}
                    accessibilityRole="button"
                  ...
                  <TouchableOpacity
                    style={[styles.copyBtn, { backgroundColor: colors.successLight, borderColor: colors.successBorder }]}
                    onPress={handleShareGuestOnWhatsApp}
                    accessibilityRole="button"
                    accessibilityLabel={isDemo ? 'Compartilhar indisponível' : 'Enviar dados no WhatsApp'}
                  >
                    <Share2 size={16} color={colors.successDark} />
                    <Text style={[styles.copyBtnText, { color: colors.successDark }]}>Enviar no WhatsApp</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
