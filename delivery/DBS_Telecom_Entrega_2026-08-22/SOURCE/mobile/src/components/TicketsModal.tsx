...
export const TicketsModal: React.FC<TicketsModalProps> = ({
  ...
  const handleCopy = async (protocol: string) => {
    ...
    const result = await copyToClipboard(protocol);
    if (result.copied) {
      onShowToast?.(result.method === 'share' ? `Protocolo ${protocol} pronto para compartilhar!` : `Protocolo ${protocol} copiado!`);
    } else {
      onShowToast?.(`Não foi possível copiar o protocolo ${protocol}.`, 'WARNING');
    }
  ...
  /** Critério único de "concluído" para filtros, contadores e badges. */
  const isTicketDone = (t: TicketRecord): boolean =>
    t.status === 'C' || t.status === 'F' || t.statusLabel === 'Concluído';

  const safeTickets: TicketRecord[] = Array.isArray(tickets) ? tickets : [];
  const ticketDataUnavailable = loadError || !isNetworkOnline;
  const filteredTickets = safeTickets.filter((t) => {
    if (filter === 'ACTIVE') return !isTicketDone(t);
    if (filter === 'DONE') return isTicketDone(t);
    return true;
  });
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityRole="button"
            accessibilityLabel="Filtrar todos os chamados"
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityRole="button"
            accessibilityLabel="Filtrar chamados em andamento"
          ...
          >
            ...
            >
              Em Andamento (
              {safeTickets.filter((t) => !isTicketDone(t)).length}
              )
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityRole="button"
            accessibilityLabel="Filtrar chamados concluídos"
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                <View style={[styles.emptyIconBox, { backgroundColor: ticketDataUnavailable ? colors.warningLight : colors.successLight }]}>
                  {ticketDataUnavailable ? <AlertCircle size={36} color={colors.warningDark} strokeWidth={2} /> : <CheckCircle2 size={36} color={colors.successDark} strokeWidth={2} />}
                </View>
                <Text style={[styles.emptyTitle, { color: colors.secondary }]}>
                  {ticketDataUnavailable ? 'Chamados indisponíveis' : 'Nenhum Chamado Encontrado'}
                </Text>
                <Text style={[styles.emptyText, { color: colors.textMuted }]}>
                  {ticketDataUnavailable ? 'Nenhum chamado foi confirmado. Tente consultar o servidor novamente.' : 'Não existem Ordens de Serviço nesta categoria. Se estiver enfrentando problemas com sua conexão, inicie um teste no chat de atendimento.'}
                </Text>
              </View>
            }
          />
