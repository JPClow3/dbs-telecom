...
export const NotificationsModal: React.FC<NotificationsModalProps> = ({
  ...
  // Guarda de formato: garante lista real antes de qualquer .filter/.map.
  const safeNotifications: PushNotification[] = Array.isArray(notifications) ? notifications : [];
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // Guarda de formato: sem isso um payload inesperado derrubaria o .filter/.map.
  ...
  const handleMarkAsRead = async (id: string) => {
    ...
    try {
      ...
      onShowToast('Não foi possível marcar como lida. Tente novamente.', 'WARNING');
    }
  };
  ...
  const handleCopyPix = async (pixCode: string) => {
    ...
    hapticFeedback.selection();
    const result = await copyToClipboard(pixCode);
    if (result.copied) {
      onShowToast(result.method === 'share' ? 'Chave PIX pronta para compartilhar! Abra o app do seu banco.' : 'Chave PIX Copiada em 1 clique! Abra o app do seu banco.');
    } else {
      onShowToast('Não foi possível copiar a chave PIX. Tente novamente.');
    }
  };
        ...
        <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.topBar, { borderBottomColor: colors.borderLight }]}>
            <Text style={[styles.unreadCount, { color: colors.textMuted }]}>
              {safeNotifications.filter((n) => !n.read).length} não lidas
            </Text>
                  ...
                  >
                    ...
                    {item.actionType === 'TICKET_DETAILS' && (
                      <TouchableOpacity
                        ...
                        accessibilityRole="button"
                        accessibilityLabel="Ver rastreio no chat"
                      >
