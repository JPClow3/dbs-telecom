...
describe('5. User Authentication & Account Sync (Password = CPF)', () => {
  ...
  it('deve rejeitar autenticação com erro genérico (anti-enumeração), sem distinguir CPF desconhecido de senha errada', async () => {
    const auth = await userService.authenticateUser('154.293.707-89', 'senha_errada_999');
    expect(auth.success).toBe(false);
    expect(auth.message).toContain('CPF ou senha inválidos');
  });
