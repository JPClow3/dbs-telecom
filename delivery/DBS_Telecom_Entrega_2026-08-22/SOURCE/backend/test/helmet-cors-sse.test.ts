...
/** Token de cliente para acompanhar o próprio stream (contrato Anti-IDOR). */
function clientToken(clientId: string, name = 'Cliente Teste'): string {
  ...
  describe('⚡ 3. Server-Sent Events (SSE) na Fila Virtual', () => {
    it('deve abrir stream SSE em /api/queue/stream/:clientId com headers adequados', (done) => {
      ...
      const req = request(app)
        ...
        .set('Authorization', `Bearer ${clientToken('2270', 'Emanuel da Silva')}`)
        .expect(200)
    ...
    it('deve emitir atualizações reativas no SSE quando a fila avança', (done) => {
      ...
      const req = request(app)
        ...
        .set('Authorization', `Bearer ${clientToken('2271', 'Cliente SSE 2')}`)
        .expect(200)
        .buffer(false);
