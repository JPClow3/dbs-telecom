...
export function validateEnv(customEnv?: NodeJS.ProcessEnv): void {
  ...
  if (nodeEnv === 'production') {
    ...
    if ((aiProvider === 'gemini' || aiProvider === 'hybrid') && isPlaceholderSecret(env.GEMINI_API_KEY)) {
      throw new Error(
        '[FATAL SECURITY CONFIG] Em produção, GEMINI_API_KEY deve ser configurada quando AI_PROVIDER usa Gemini.'
      );
    }
    if ((aiProvider === 'openai' || aiProvider === 'hybrid') && isPlaceholderSecret(env.OPENAI_API_KEY)) {
      throw new Error(
        '[FATAL SECURITY CONFIG] Em produção, OPENAI_API_KEY deve ser configurada quando AI_PROVIDER usa OpenAI.'
      );
    }
