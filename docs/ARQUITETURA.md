...
* **Tier 2 — Google Gemini Provider com RAG Dinâmico:**
  - Modelo `gemini-flash-lite-latest` / `gemini-3.5-flash-lite` via Google AI Studio REST API.
...
sequenceDiagram
    ...
    IXC-->>UserSvc: Retorna registro do cliente (ID 2270, Nome: "Emanuel da Silva")
...
```
backend/
├── Dockerfile                  # Empacotamento Docker multi-stage (Node 20 Alpine)
...
1. **Timeout e Cancelamento com `AbortSignal`:** Todas as chamadas ao IXC Soft e ao Google Gemini utilizam timeouts estritos (10s para o IXC e 12s para o Gemini) para impedir travamento de threads no backend.
2. **Failover de Modelos do Gemini:** O `GeminiProvider` possui lista ordenada de modelos candidatos (`gemini-flash-lite-latest`, `gemini-3.1-flash-lite`, `gemini-3.5-flash-lite`, `gemini-3.5-flash`), alternando automaticamente em caso de indisponibilidade momentânea de um endpoint.
